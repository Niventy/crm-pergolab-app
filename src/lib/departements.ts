// Noms des départements (code → nom) pour afficher « 31 · Haute-Garonne » à
// partir d'un code postal : c'est le premier critère « dans la zone / hors zone ».
export const DEPT_NOMS: Record<string, string> = {
...
export function departementDe(codePostal: string | null | undefined): { code: string; nom: string } | null {
  ...
  else if (code === "20") code = Number(cp) < 20200 ? "2A" : "2B";
  const nom = DEPT_NOMS[code];
  return nom ? { code, nom } : { code, nom: "Département inconnu" };
}
